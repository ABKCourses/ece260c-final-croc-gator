...
utl::report "Using fallback dimensions for small macro"
set macro_width 650.0   ;# microns - adjust based on your register file size
set macro_height 650.0  ;# microns


# Add margin for power ring and pin access
set margin 25.0
set total_width  [expr $macro_width + 2 * $margin]
set total_height [expr $macro_height + 2 * $margin]

utl::report "Macro dimensions: ${total_width} x ${total_height} microns"

utl::report "Initialize macro floorplan"
initialize_floorplan -die_area  "0 0 $total_width $total_height" \
                     -core_area "$margin $margin [expr $total_width - $margin] [expr $total_height - $margin]" \
                     -site "CoreSite"
makeTracks
###############################################################################
# PIN PLACEMENT                                                               #
###############################################################################
utl::report "###############################################################################"
utl::report "# Automatic pin placement"
utl::report "###############################################################################"


# First, let's check if we have any unplaced pins
    
    # Place pins with more specific layer assignments and better spacing
...
# read_def /fosic/designs/croc/openroad_rf/out/cve2_register_file_ff.def -incremental
...
global_placement -skip_io {*}$GPL2_ARGS
...
set_global_routing_layer_adjustment Metal5 0.15
...
repair_antennas -ratio_margin 25 -iterations 4
# check_antennas
